...
import logging
logging.basicConfig(filename='willywonka.log',level=logging.INFO)
    ...
    def build_whole_detection_network(self, input_img_batch, gtboxes_batch, gtboxes_r_batch, gpu_id):
        ...
        if self.is_training:
            with tf.variable_scope('sample_anchors_minibatch'):
                ...
                #logger.info('fpn_labels is type'+str(type(tf.make_ndarray(fpn_labels.op.get_attr('value')))))
                ...
                #logger.info('fpn_labels is type'+str(type(tf.convert_to_tensor(fpn_labels, dtype=tf.float32))))
