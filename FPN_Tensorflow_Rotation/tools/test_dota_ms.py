...
print('The checkpoint folders are: ', cfgs.TRAINED_CKPT, cfgs.VERSION)
...
def worker(gpu_id, images, det_net, args, result_queue):
    ...
    img_batch = short_side_resize_for_inference_data(img_tensor=img_batch,
                                                     ...
                                                     is_resize=not args.multi_scale) # this is skipped if we ARE doing multi-scale
...
#     with open('/home/on22851/Old_Before_Working_Directory/DOTA-DOAI-master/FPN_Tensorflow_Rotation/tools/LOOKATMEEEEE.txt','a') as fileobject:
#         fileobject.write('detection_boxes is'+' '+str(type(detection_boxes))+' '+str(detection_scores.get_shape().as_list())+'\n')
#     fileobject.close
    ...
    with tf.Session(config=config) as sess:
        ...
        for img_path in images:
            ...
            threshold = {'class1':0.3,'class2':0.3,'class3':0.3}
...
def test_dota(det_net, real_test_img_list, args, txt_name):
    ...
    save_path = os.path.join(args.write_dir, cfgs.VERSION)
    ...
    for i in range(nr_records):
        ...
        if args.show_box:
            ...
            tools.mkdir(os.path.join(save_path, 'myproject_img_vis'))
            draw_path = os.path.join(save_path, 'myproject_img_vis', nake_name)
            ...
            tools.mkdir(os.path.join(save_path, 'myproject_res'))
            for sub_class in CLASS_DOTA:
                if sub_class == 'back_ground':
                    continue
                write_handle[sub_class] = open(os.path.join(save_path, 'myproject_res', 'Task1_%s.txt' % sub_class), 'a+')
...
def parse_args():
    ...
    parser.add_argument('--write_dir', dest='write_dir',
                        help='dir where results are written',
                        default='./test_myproject_large_chip/', type=str)
